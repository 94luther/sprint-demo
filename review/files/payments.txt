...
export interface Payment {
  ...
  /** every refund under its own key, so a retried refund is the SAME refund */
...
export function refund(book: PaymentBook, key: string, amountThebe: number, refundKey: string): Payment {
  const p = book.get(key);
  if (!p) throw new PaymentError('No payment with that key.');
  /* Found by Codex on 16 Sep 2026: a refund had no key of its own, so a customer
     on a bad line who tapped Refund twice got P40 back for a P20 refund. The same
     rule as taking the money: the same key is the same refund, never a second. */
  if (!refundKey) throw new PaymentError('A refund needs its own key, so a retried refund is not a second refund.');
  if (!p.refunds) p.refunds = {};
  if (refundKey in p.refunds) {
    if (p.refunds[refundKey] !== amountThebe) {
      throw new PaymentError('This refund key has already been used for a different amount. Refusing.');
    }
    return p; // the retried tap
  }
  ...
  p.refunds[refundKey] = amountThebe;
